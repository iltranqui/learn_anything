...
ICinputs = {'u_{force}','u_{lamearia}'};  % Input channel names for initial conditions
ICoutputs = {'Output'}; % Output channel names for initial conditions
Sum = sumblk('Output = y_{position} + y_{lamearia}'); % Summation block for initial conditions

model = connect(G,D,Sum,ICinputs,ICoutputs);

% bode transfer function of the system
figure(1)
bode(model)
grid on
