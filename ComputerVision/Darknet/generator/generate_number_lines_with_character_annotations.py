"""
Number Line Dataset Generator with Character-Level YOLO Annotations

This script generates a dataset of images with lines of random numbers (0-9)
and corresponding YOLO format annotations for each individual character.
Each image contains 3 lines of text, with 10 numbers per line.

Usage:
    python generate_number_lines_with_character_annotations.py [options]
...
def parse_args():
    parser = argparse.ArgumentParser(description='Generate a dataset of images with lines of random numbers (0-9) and character-level YOLO annotations')
    parser.add_argument('--output_dir', type=str, default='data/number_lines_character_yolo', help='Output directory')
...
def generate_image_with_character_annotations(img_index, font_files, args):
    """Generate an image with 3 lines of random numbers (0-9) and character-level YOLO annotations"""
    ...
    for i in range(3):
        ...
        char_width = text_width / len(line)
        
        # Generate annotations for each character
        for j, char in enumerate(line):
            # Calculate character position
            char_x = x + j * char_width
            char_y = y
            
            # Calculate YOLO format annotation for this character
            # YOLO format: <class_id> <x_center> <y_center> <width> <height>
            # All values are normalized to [0, 1]
            class_id = int(char)  # Class ID is the digit itself (0-9)
            x_center = (char_x + char_width / 2) / args.image_width  # Normalized x-center
            y_center = char_y / args.image_height  # Normalized y-center
            width = char_width / args.image_width  # Normalized width
...
def create_dataset_files(args):
    ...
    with open(names_file, 'w') as f:
        for i in range(10):  # Write digits 0-9
            f.write(f"{i}\n")
...
def main():
    ...
    # Print configuration
    print(f"Number Line Dataset Generator with Character-Level YOLO Annotations")
    print(f"=============================================================")
    ...
    for i in range(args.num_images):
        img_path, lines, annotations = generate_image_with_character_annotations(i, font_files, args)
        ...
        print(f"  Character Annotations (showing first 5):")
        for ann in annotations[:5]:
            print(f"    {ann}")
        print(f"    ... ({len(annotations) - 5} more annotations)")
    ...
    print(f"Character-level YOLO annotations saved directly with each image")
